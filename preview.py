...
def plot_subject(ct_img, output_path, label_id=1, window_size=[500, 500], background=[1.0, 1.0, 1.0], smoothing=10, color=[255.0, 193.0, 149.0], orientation='sagittal',projection='orthographic'):
    scene = window.Scene()
    scene.background(background)
    window.ShowManager(scene, size=window_size,
                       reset_camera=False).initialize()
    ...
    window.record(scene, size=window_size,
                  out_path=output_path, reset_camera=False)
    scene.clear()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--projection',choices=['parallel','orthographic'],default='orthographic')
